  ...
  <div class="out-container">
    <div class="recode-container">
      ...
      <div class="recode-input-row">
        <span
          class="recode-box"
          v-clipboard:copy="new_recode"
          v-clipboard:success="copysuccess"
          v-clipboard:error="copyerror"
          v-if="!is_editting_recode"
          >{{ new_recode }}</span
        >
        <input
          id="recode-box"
      ...
      <p
        style="font-size: 12px; color: grey; margin: 8px 0;"
        v-if="!is_editting_recode"
      >
        点击提取码一键复制
      </p>
      ...
      <div class="more-setting-container">
        ...
        <div class="download-count-btn-group">
          <a-radio-group buttonStyle="solid" v-model="download_count">
            <a-radio-button class="download-count" value="1"
              >1 次</a-radio-button
            >
            <a-radio-button class="download-count" value="10"
              >10 次</a-radio-button
            >
            <a-radio-button class="download-count" value="20"
              >20 次</a-radio-button
            >
            <a-radio-button class="download-count" value="50"
              >50 次</a-radio-button
            >
          </a-radio-group>
        </div>
        ...
        <div class="expire-time-btn-group">
          <a-radio-group buttonStyle="solid" v-model="expire_time">
            <a-radio-button class="expire-time" value="4"
              >4 小时</a-radio-button
            >
            <a-radio-button class="expire-time" value="8"
              >8 小时</a-radio-button
            >
            <a-radio-button class="expire-time" value="12"
              >12 小时</a-radio-button
            >
            <a-radio-button class="expire-time" value="24"
              >24 小时</a-radio-button
            >
          </a-radio-group>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      is_need_password:
        JSON.parse(window.localStorage.getItem("recode-" + this.recode))
          .password != null &&
        JSON.parse(window.localStorage.getItem("recode-" + this.recode))
          .password != "",
      password:
  ...
  computed: {
    copy_url: function () {
      return encodeURI(
        _global.domain_url + "download/" + this.recode + "?from=copyurl"
      );
    },
    qrcode_url: function () {
      return encodeURI(
        _global.domain_url + "download/" + this.recode + "?from=qrcode"
      );
    },
  },
  mounted() {
  ...
  methods: {
    ...
    finisheditrecode() {
      ...
      if (this.recode != this.new_recode) {
        ...
        var user_token = JSON.parse(
          window.localStorage.getItem("recode-" + this.recode)
        ).owner_token;
        xhr.open("POST", _global.api_url + "recode/" + this.recode, true);
        xhr.onreadystatechange = function () {
          if (xhr.readyState == XMLHttpRequest.DONE) {
            if (xhr.status == 200) {
              var uploadedinfo = JSON.parse(
                window.localStorage.getItem("recode-" + that.recode)
              );
              var href = window.location.href;
              window.history.pushState(
                {},
                0,
                href.split("code=")[0] + "code=" + that.new_recode
              );
              ...
              window.localStorage.setItem(
                "recode-" + that.new_recode,
                JSON.stringify(uploadedinfo)
              );
    ...
    submit() {
      ...
      xhr_password.open(
        "POST",
        _global.api_url + "password/" + this.recode,
        true
      );
      xhr_downloadcount.open(
        "POST",
        _global.api_url + "downCount/" + this.recode,
        true
      );
      xhr_expiretime.open(
        "POST",
        _global.api_url + "expireTime/" + this.recode,
        true
      );
      ...
      xhr_password.send(
        JSON.stringify({ user_token: user_token, Auth: password_sha256 })
      );
      xhr_downloadcount.setRequestHeader("X-CSRF-TOKEN", csrf_token);
      xhr_downloadcount.send(
        JSON.stringify({
          new_down_count: parseInt(that.download_count),
          user_token: user_token,
        })
      );
      ...
      xhr_expiretime.send(
        JSON.stringify({
          user_token: user_token,
          new_expire_time: parseInt(that.expire_time),
        })
      );
    },
  },
