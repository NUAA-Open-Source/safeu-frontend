  ...
  <div class="uploadedfiles-container" v-if="uploaded_files.length > 0">
    ...
    >
      <template slot="recode" slot-scope="recode">
        <a v-on:click="gotoedit(recode.recode)" v-if="!recode.isExpired">{{
          recode.recode
        }}</a>
        <span v-else>{{ recode.recode }}</span>
      </template>
      <template slot="download_url" slot-scope="download_url">
        <a v-on:click="gotodownload(download_url)" v-if="download_url != null"
          >提取</a
        >
        <span v-else>提取</span>
      </template>
      ...
      <template slot="delete" slot-scope="code">
        <a v-on:click="showmodal(code.recode)" v-if="!code.isExpired"
          ><font-awesome-icon icon="trash"
        /></a>
        <a v-on:click="removels(code.recode)" style="color: red;" v-else
...
export default {
  ...
  mounted() {
    ...
    for (var i = 0; i < localStorage.length; i++) {
      ...
      if (key.indexOf("recode-") == 0) {
        ...
        var remain_time =
          createdAt + expire_time * 60 * 60 * 1000 - Date.parse(new Date());
        if (remain_time < 0) {
          this.uploaded_files.push({
            recode: { recode: recode, isExpired: true },
            download_url: null,
            remain_time: null,
            code: { recode: recode, isExpired: true },
            createdAt: null,
          });
        } else {
          var remain_hour = parseInt(remain_time / 1000 / 60 / 60).toString();
          var remain_min = Math.round(
            (remain_time / 1000 / 60) % 60
          ).toString();
          this.uploaded_files.push({
            recode: { recode: recode, isExpired: false },
            download_url: recode,
            remain_time: remain_hour + "小时" + remain_min + "分钟",
            code: { recode: recode, isExpired: false },
            createdAt: createdAt,
          });
        }
      }
  ...
  methods: {
    gotoedit(recode) {
      this.$router.push({
        path: "/recode",
        query: { code: recode, from: "history" },
      });
    },

    gotodownload(recode) {
      this.$router.push({
        path: "/download/" + recode,
        query: { from: "history" },
      });
    },
    ...
    deletefile(recode) {
      var user_token = JSON.parse(
        window.localStorage.getItem("recode-" + recode)
      ).owner_token;
